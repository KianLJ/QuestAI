...
async function callQuestAI(prompt, timeoutMs = 60000) {
  ...
  if (response.status === 400) throw new Error("Invalid Gemini API key — check Settings.");
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 20px 0" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22, flexWrap: "wrap", gap: 10 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
            ...
            <button onClick={() => { setGeminiKeyInput(getGeminiKey()); setSettingsOpen(true); }} className="qlog-btn" aria-label="Settings" style={{ background: "#232E3D", border: "1px solid #33414F", borderRadius: 8, padding: "8px 10px", color: "#8A8578", cursor: "pointer" }}><Gear size={16} /></button>
        ...
        {settingsOpen && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(10,14,20,0.7)", zIndex: 70, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div style={{ background: "#232E3D", border: "1px solid #33414F", borderRadius: 16, padding: 24, width: "100%", maxWidth: 380, position: "relative" }}>
              ...
              <p style={{ fontSize: 12, color: "#8A8578", margin: "0 0 14px" }}>Add your Gemini API key to enable AI features (auto-difficulty, brain dump, epic split). Get a free key at aistudio.google.com. Stored only in your browser.</p>
              <input type="password" value={geminiKeyInput} onChange={(e) => setGeminiKeyInput(e.target.value)} placeholder="AIza..." style={{ width: "100%", marginBottom: 12, background: "#141C27", border: "1px solid #33414F", borderRadius: 8, padding: "10px 12px", color: "#EDE4D3", fontSize: 13, fontFamily: "ui-monospace, Menlo, monospace" }} />
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => { saveGeminiKey(geminiKeyInput); setSettingsOpen(false); }} className="qlog-btn" style={{ flex: 1, background: accent, border: "none", borderRadius: 8, padding: "10px 0", fontWeight: 700, color: "#1B2430", cursor: "pointer" }}>Save</button>
                <button onClick={() => { saveGeminiKey(""); setGeminiKeyInput(""); }} className="qlog-btn" style={{ background: "#141C27", border: "1px solid #33414F", borderRadius: 8, padding: "10px 14px", color: "#8A8578", cursor: "pointer" }}>Clear</button>
              </div>
              <p style={{ fontSize: 11, color: "#5C6773", margin: "12px 0 0" }}>The free tier is generous enough for light personal use. No credit card needed.</p>
            </div>
          </div>
        )}
